...
SYS_RELEVANT_PRODUCT_USER = 'Respond to the user with the product that is most relevant to advertise to the user based on their demographics and prompt. You are only allowed to reply with exactly that product. The user\'s profile is here: {profile}.\n\n The list of products is here: {products}. The descriptions of each product are here: {descs}.'
...
SYS_USER_PROFILE_SUMMARY = 'The user will provide a list of prompts consisting of questions, tasks, and requests they have asked an AI assistant to handle. Use the information and clues within these prompts to respond with a forensic analysis and creation of a detailed demographic, interest, behavioral, personality, psychological forensic analysis and user profile. Return JSON profile containing a comprehensive wholistic profile for a forensic analysis of the person. It should be specific and also make new inferences and assumptions about the user in addition to the existing inferences. Return a JSON. Each should have the format like so:\n\n{\n "category": {\n   "category_attribute": [\n        "Specific qualifier1", "Specific qualifier2", ...     ]\n}\n\n" It must include demographics (age, gender, location, occupation, ethicity, etc.) and interests, and should also include behavioral, personality, and psychological traits.'
