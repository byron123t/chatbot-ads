...
SYS_RELEVANT_PRODUCT_USER = 'Respond to the user with the product that is most relevant to advertise to the user based on their demographics and prompt. You are only allowed to reply with exactly that product. The user\'s demographics are here. \n- Age: {age}\n- Gender: {gender}\n- Relationship: {relationship}\n- Race: {race}\n- Interests: {interests}\n- Occupation: {occupation}\n- Politics: {politics}\n- Religion: {religion}\n- Location: {location}\n\n The list of products is here: {products}. The descriptions of each product are here: {descs}.'
