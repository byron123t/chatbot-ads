...
class Advertiser:
    def __init__(self, mode:str='control', session:str='', ad_freq:float=1.0, conversation_id:str='', profile:str='', self_improvement:int=None, feature_manipulation:bool=False, verbose:bool=False, EVAL:bool=False):
        self.oai_api = OpenAIAPI()
        self.mode = mode
        self.system_prompt = ''
        self.products = Products(verbose=verbose)
        self.topics = Topics(verbose=verbose)
        self.chat_history = ChatHistory(verbose=verbose, session=session, conversation_id=conversation_id, EVAL=EVAL)
        self.personality = ''
        ...
        if mode == 'interest-based':
            ...
            self.initializer = prompts.SYS_USER_CENTRIC_INTEREST_ADAPT
            self.initializer_desc = prompts.SYS_USER_CENTRIC_INTEREST_ADAPT_DESC
        else:
    ...
    def parse(self, prompt:str):
        ...
        topic = None
        if not self.product[self.conversation_id]['name'] or not self.check_relevance(prompt, self.product) or self.EVAL:
        ...
        if self.EVAL and not self.product[self.conversation_id]['name']:
            if not topic:
                topic = self.topics.find_topic(prompt)
            if not topic:
                topic_keys = self.topics.topics.keys()
                selected_topic = random.choice(list(topic_keys))
